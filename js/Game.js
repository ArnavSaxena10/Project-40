class Game{
    constructor(){

    }
    getState() {
        var gameStateRef = database.ref('gameState');
        gameStateRef.on("value", function (data) {
            gameState = data.val();
        })

    }

    update(state) {
        database.ref('/').update({
            gameState: state
        });
    }
    async start() {
            if (gameState === 0) {
                player = new Player();
                var playerCountRef = await database.ref('playerCount').once("value");
                if (playerCountRef.exists()) {
                    playerCount = playerCountRef.val();
                    player.getCount();
                }
                form = new Form()
                form.display();
            }
    player1 = createSprite(200,500);
    player1.addImage("player1",player_img);
    
    player2 = createSprite(800,500);
    player2.addImage("player2", player_img);
    players=[player1,player2];

        }
    
    play(){
        image(back_img, 0, 0, 1000, 800);  
        form.hide();
        Player.getPlayerInfo();

        if(allPlayers !== undefined){
        var x =100;
        var y=200;
        var index =0;

        if(player.score>=20){
            gameState=2
            this.update(gameState);
        }
                      
        for(var plr in allPlayers){

        index = index+1;
        x = 500-allPlayers[plr].distance;
        y=500;           
        players[index -1].x = x;
        players[index - 1].y = y;
        
        if(index === player.index){     
        players[index - 1].shapeColor = "red";          
        stroke(10);
        fill("red");
        ellipse(x,y,60,60); 
        textSize(20);
        fill("red");
        text(allPlayers[plr].name+": " +player.score, 100,50);
        //text("Player"+player.index+": " +player.score, 100,50);
        
        if(players[index-1].isTouching(fruit1Group)){
            fruit1Group.destroyEach();
            player.score+=1;
            player.update();
        }

        if(players[index-1].isTouching(fruit2Group)){
            fruit2Group.destroyEach();
            player.score+=1;
            player.update();
        }

        if(players[index-1].isTouching(fruit3Group)){
            fruit3Group.destroyEach();
            player.score+=1;
            player.update();
        }

        if(players[index-1].isTouching(fruit4Group)){
            fruit4Group.destroyEach();
            player.score+=1;
            player.update();
        }

        if(players[index-1].isTouching(fruit5Group)){
            fruit5Group.destroyEach();
            player.score+=1;
            player.update();
        }
       }
       else{
        textSize(20);
        fill("white");
        text(allPlayers[plr].name+": "+allPlayers[plr].score ,100,90);
        //text("Player"+index+": "+allPlayers[plr].score, 100,90);
       }       
    }
    }

        if (keyIsDown(RIGHT_ARROW) && player.index !== null) {
            player.distance -= 10
            player.update();
        }
        if (keyIsDown(LEFT_ARROW) && player.index !== null) {
            player.distance += 10
            player.update();
        }
            
        // if (frameCount % 20 === 0) {
        //     fruits = createSprite(random(100, 1000), 0, 100, 100);
        //     fruits.velocityY = 6;
        //     var rand = Math.round(random(1,5));
        //     switch(rand){
        //         case 1: fruits.addImage("fruit1",fruit1_img);
        //         break;
        //         case 2: fruits.addImage("fruit1", fruit2_img);
        //         break;
        //         case 3: fruits.addImage("fruit1", fruit3_img);
        //         break;
        //         case 4: fruits.addImage("fruit1", fruit4_img);
        //         break;
        //         case 5: fruits.addImage("fruit1", fruit5_img);
        //         break;
        //     }
        // fruitGroup.add(fruits);
                     
        // }
                 
        if (player.index !== null) {
        //fill code here, to destroy the objects.
        
        }

        drawSprites();

    }

    end(){
       console.log("Game Ended");
    }
}